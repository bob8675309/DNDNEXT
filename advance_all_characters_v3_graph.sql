-- DNDNext Option A (Graph Edges) - Phase 3
-- Patch advance_all_characters_v3 to choose next_point_seq via route_next_seq_pick_v2
-- (legacy behavior unchanged until map_routes.use_graph = true)
...
WHERE is_hidden = false
  ...
  LOOP
    ...
    IF c.state = 'resting' AND c.location_id IS NOT NULL THEN
      ...
      c.next_point_seq := public.route_next_seq_pick_v2(
        c.route_id,
        c.current_point_seq,
        c.prev_point_seq,
        c.route_mode,
      ...
      -- If no next point, stop routing and remain resting here.
      ...
      SET state = CASE WHEN c.route_mode = 'excursion' THEN 'excursion' ELSE 'moving' END,
          ...
          prev_point_seq = c.current_point_seq,
    ...
    IF c.state IN ('moving','excursion') THEN
      ...
      IF c.segment_ends_at IS NOT NULL AND c.segment_ends_at <= p_world_time THEN
        -- Compute next seq using graph/legacy selector
        v_next_seq := public.route_next_seq_pick_v2(
          c.route_id,
          c.next_point_seq,
          c.current_point_seq,
          c.route_mode,
        ...
        IF bloc IS NOT NULL THEN
          ...
          SET state = 'resting',
              ...
              prev_point_seq = CASE WHEN v_next_seq IS NULL THEN NULL ELSE c.current_point_seq END,
              location_id = bloc,
              projected_destination_id = NULL,
...
-- Convenience wrapper (if you use it)
