...
/**
 * TradeRequestsPanel
 * - Shows incoming/outgoing trade requests for the signed-in user.
 * - Looks up the related inventory item (for name/thumb).
 */
export default function TradeRequestsPanel() {
  ...
  async function load(userId) {
    ...
    // Fetch the items referenced by either list
    const ids = Array.from(
      new Set([...(inc.data || []), ...(out.data || [])].map((r) => r.inventory_item_id))
    );
    if (ids.length) {
      ...
      (items || []).forEach((i) => {
        const payload =
          typeof i.card_payload === "string"
            ? safeParse(i.card_payload) || {}
            : i.card_payload || {};
        map[i.id] = {
          ...i,
          // Prefer JSONB thumbnail; fall back to a placeholder
    ...
    <div className="card">
      <div className="card-header d-flex justify-content-between align-items-center">
        <div className="btn-group" role="group">
          <button
            className={`btn btn-sm ${tab === "incoming" ? "btn-primary" : "btn-outline-primary"}`}
            onClick={() => setTab("incoming")}
          >
            Incoming
          </button>
          <button
            className={`btn btn-sm ${tab === "outgoing" ? "btn-primary" : "btn-outline-primary"}`}
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <div className="flex-grow-1">
                  ...
                  <div className="small text-muted">
                    {tab === "incoming" ? "From" : "To"}: {who?.slice(0, 8)}… •{" "}
                ...
                {r.status === "pending" && tab === "incoming" && (
                  <div className="btn-group">
                    <button className="btn btn-sm btn-success" onClick={() => accept(r.id)} disabled={busy}>
                      Accept
                    </button>
                    <button
