...
export default function LocationIconDrawer({
  ...
  // IMPORTANT:
  // Do NOT stop events during the CAPTURE phase here. Doing so prevents clicks from reaching
  // child controls (tabs, buttons, list rows) and makes the drawer feel "unclickable".
  // We still stop propagation (BUBBLE phase) so map-level handlers underneath don’t react.
  return (
    <div
    ...
    >
      <div className="loc-drawer__header">
        {(() => {
          ...
          // - NPC tab: "NPC Markers" and show the selected NPC name below
          ...
          const subtitle = isNpcTab ? (found?.name || "No NPC selected") : null;
          return (
            <>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                <div className="loc-drawer__title">{headerTitle}</div>
                {subtitle ? <div style={{ opacity: 0.85, fontSize: 12 }}>{subtitle}</div> : null}
              </div>
              <button
                type="button"
