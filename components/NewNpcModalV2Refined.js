...
const STEP_LABELS = Object.freeze(["Species", "Background", "Class", "Abilities", "Training", "Story", "Identity", "Review"]);
...
function DiceSummary({ roll, index, assignedAbility, selected, onSelect }) {
  return <button type="button" draggable className={`npc-forge-roll-card refined ${selected ? "is-selected" : ""}`} onClick={() => onSelect(roll.id)} onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/npc-forge-roll", roll.id); onSelect(roll.id); }} aria-pressed={selected}><small>Die Roll {index + 1}</small><strong>{roll.total}</strong><div>{roll.dice.map((die, dieIndex) => <span key={`${roll.id}-${dieIndex}`} className={dieIndex === roll.droppedIndex ? "is-dropped" : ""}>{die}</span>)}</div><em>{assignedAbility ? `Assigned to ${ABILITY_LABELS[assignedAbility]}` : "Drag or select to assign"}</em></button>;
}
...
export default function NewNpcModalV2({ show, onClose, onCreated, locations = [] }) {
  ...
  const createPayload = useMemo(() => {
    ...
    return { ...base, name: safeText(draft.name), race: speciesName, role: safeText(draft.role) || (draft.kind === "merchant" ? "Merchant" : className), affiliation: safeText(draft.affiliation) || null, sheet };
  ...
  function resetForm() {
    ...
    setRolls(nextRolls); setAllocation(defaultRollAllocation(nextRolls)); setSelectedRollId(""); setDetail(null);
  }
  ...
  function handleAbilityDrop(event, ability) {
    event.preventDefault();
    const rollId = event.dataTransfer.getData("text/npc-forge-roll") || selectedRollId;
    allocateRoll(ability, rollId);
  }
  function assignSelectedRoll(ability) {
    if (selectedRollId) allocateRoll(ability, selectedRollId);
    else setDetail({ type: "ability", key: ability });
  }
  function setAbility(key, value) {
    setDraft((current) => ({ ...current, abilityMethod: "manual", baseAbilities: { ...(current.baseAbilities || {}), [key]: Math.max(1, Math.min(30, Number(value) || 1)) } }));
    setDetail({ type: "ability", key }); setError("");
  }
  ...
  function togglePlusOne(ability) {
    setDraft((current) => { const selected = uniqueText(current.backgroundBoosts?.plusOnes || []); const next = selected.includes(ability) ? selected.filter((key) => key !== ability) : selected.length < 3 ? [...selected, ability] : selected; return { ...current, backgroundBoosts: { ...(current.backgroundBoosts || {}), mode: "three", plusOnes: next, allowAny: true } }; });
    setDetail({ type: "ability", key: ability }); setError("");
  }
  ...
  function toggleClassSkill(skillKey) {
    setDraft((current) => { const selected = uniqueText(current.selectedClassSkills || []); const next = selected.includes(skillKey) ? selected.filter((key) => key !== skillKey) : selected.length < classSkillConfig.count ? [...selected, skillKey] : selected; return { ...current, selectedClassSkills: next }; });
    setDetail({ type: "skill", key: skillKey }); setError("");
  }
  ...
  function setProfession(professionKey, field, value) {
    setDraft((current) => ({ ...current, professions: { ...(current.professions || {}), [professionKey]: { ...(current.professions?.[professionKey] || {}), [field]: value, ...(field === "rank" && Number(value) === 0 ? { offersService: false } : {}) } } }));
    setDetail({ type: "profession", key: professionKey }); setError("");
  }
  ...
  function generateStory() {
    ...
    const generated = generateNpcStory({ locations, species: selectedSpecies, background: selectedBackground, classRow: selectedClass, skills: selectedSkillKeys, professions, level: draft.level });
    patch(generated);
  }
  ...
  function stepErrors(index) {
    ...
    if (index === 6) { if (!safeText(draft.name)) errors.push("Enter or generate a name."); if (!safeText(draft.role)) errors.push("Enter a role or title so the roster remains useful."); }
    return errors;
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      resetForm(); await onCreated?.(created);
    } catch (err) { setCreating(false); setError(String(err?.message || err || "Failed to create character.")); }
  }
  ...
  return <div className="npc-forge-backdrop" role="presentation"><div className="npc-forge-modal npc-forge-modal-v2" role="dialog" aria-modal="true" aria-labelledby="npc-forge-title">
    <header className="npc-forge-header"><div><div className="npc-forge-kicker">Canonical character system</div><h2 id="npc-forge-title">NPC Forge</h2><p>Build the rules first, then finish identity and placement. The right column explains and completes the choice currently being made.</p></div><button type="button" className="btn btn-sm btn-outline-light" onClick={handleClose} disabled={creating}>Close</button></header>
    ...
    <div className="npc-forge-body"><section className="npc-forge-workspace">{catalogError ? <div className="npc-forge-catalog-warning">{catalogError}</div> : null}
    ...
    {error ? <div className="npc-forge-error" role="alert">{error}</div> : null}<footer className="npc-forge-footer"><button type="button" className="btn btn-outline-light" onClick={handleClose} disabled={creating}>Cancel</button><div>{step > 0 ? <button type="button" className="btn btn-outline-light" onClick={handleBack} disabled={creating}>Back</button> : null}{step < STEP_LABELS.length - 1 ? <button type="button" className="btn btn-primary" onClick={handleNext} disabled={creating || loadingCatalogs}>Continue</button> : <button type="button" className="btn btn-success" onClick={handleCreate} disabled={creating}>{creating ? "Forging Character..." : `Create ${draft.kind === "merchant" ? "Merchant" : "NPC"}`}</button>}</div></footer>
    <style jsx global>{`
      .npc-forge-modal-v2 .npc-forge-body{grid-template-columns:minmax(0,57fr) minmax(470px,43fr)}.npc-forge-workspace-note{padding:11px 13px;border-left:3px solid #58d6c7;border-radius:8px;color:rgba(255,255,255,.66);background:rgba(88,214,199,.07);font-size:.76rem;line-height:1.5}.npc-forge-roll-card.refined{appearance:none;width:100%;cursor:grab;text-align:center}.npc-forge-roll-card.refined:active{cursor:grabbing}.npc-forge-roll-card.refined>small{color:#f3bf63;font-size:.65rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase}.npc-forge-roll-card.refined>em{min-height:28px;color:rgba(255,255,255,.48);font-size:.61rem;font-style:normal;line-height:1.25}.npc-forge-roll-card.refined.is-selected{border-color:#a86cff;box-shadow:0 0 0 3px rgba(168,108,255,.18),inset 0 0 20px rgba(168,108,255,.12);transform:translateY(-2px)}.npc-forge-allocation-instruction{margin:12px 0 8px;padding:9px 11px;border-radius:8px;color:#d9c5fa;background:rgba(126,72,199,.1);font-size:.72rem}.npc-forge-ability-drop-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.npc-forge-ability-drop-grid button{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:3px 10px;align-items:center;min-height:82px;padding:12px;border:1px solid rgba(255,255,255,.1);border-radius:11px;color:rgba(255,255,255,.72);background:rgba(255,255,255,.026);text-align:left;transition:border-color .12s,background .12s,transform .12s}.npc-forge-ability-drop-grid button:hover,.npc-forge-ability-drop-grid button.is-ready{border-color:rgba(168,108,255,.48);background:rgba(126,72,199,.08)}.npc-forge-ability-drop-grid button.is-ready:hover{transform:translateY(-2px);border-color:#a86cff}.npc-forge-ability-drop-grid span{color:#fff;font-size:.8rem;font-weight:800}.npc-forge-ability-drop-grid strong{grid-row:1/3;grid-column:2;color:#fff3ce;font-size:1.45rem}.npc-forge-ability-drop-grid small{color:#cfb4f7;font-size:.68rem}.npc-forge-ability-drop-grid em{grid-column:1/-1;color:rgba(255,255,255,.52);font-size:.65rem;font-style:normal}.npc-forge-story-actions{display:grid;justify-items:end;gap:5px}.npc-forge-story-actions button{padding:7px 11px;border:1px solid rgba(88,214,199,.44);border-radius:8px;color:#c9fff7;background:rgba(42,136,124,.12);font-size:.72rem;font-weight:800}.npc-forge-story-actions button:hover{border-color:#58d6c7;background:rgba(42,136,124,.2)}.npc-forge-story-actions p{margin:0}.npc-forge-world-fit{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 0 14px;padding:10px 12px;border:1px solid rgba(88,214,199,.22);border-radius:9px;background:rgba(88,214,199,.07)}.npc-forge-world-fit strong{color:#bdfbf2;font-size:.75rem}.npc-forge-world-fit span{color:rgba(255,255,255,.62);font-size:.68rem}@media(max-width:1220px){.npc-forge-modal-v2 .npc-forge-body{grid-template-columns:minmax(0,3fr) minmax(380px,2fr)}.npc-forge-ability-drop-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:980px){.npc-forge-modal-v2 .npc-forge-body{grid-template-columns:1fr}.npc-forge-ability-drop-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:720px){.npc-forge-ability-drop-grid{grid-template-columns:1fr}.npc-forge-story-actions{justify-items:start}.npc-forge-world-fit{display:grid}}
    `}</style>
  </div></div>;
}
