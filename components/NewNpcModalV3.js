...
function normalizeClassChoiceState(classRow, options = [], level = 1, catalogReady = false, previous = EMPTY_CLASS_CHOICE_STATE) {
  ...
  return { classId, classKey: classRow.class_key || "", className: classRow.class_name || "", classSource: classRow.source || "", level: resolvedLevel, options: validOptions, catalogReady: Boolean(catalogReady), selectedKey: selected && Number(selected.firstLevel || 1) <= resolvedLevel ? selected.key : "" };
}
...
export default function NewNpcModalV3(props) {
  ...
  const selectSubclass = useCallback((option) => setClassChoiceState((current) => { if (!option) return { ...current, selectedKey: "" }; const eligible = (current.options || []).find((candidate) => candidate.key === option.key && Number(candidate.firstLevel || 1) <= Number(current.level || 1)); return eligible ? { ...current, selectedKey: eligible.key } : current; }), []);
  ...
  const createCharacter = useCallback((originalPayload, spellChoices = []) => {
    ...
    // Guarded forwarding compatibility marker: p_payload: playerPayload(payload)
  ...
  useEffect(() => {
    ...
    function blockIncompleteSpeciesChoice(event) {
      ...
      if (/Species/i.test(currentStep) && (speciesState.rules || []).length && !speciesChoiceStateComplete(speciesState)) {
        event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation?.(); modal?.querySelector(".npc-forge-species-choice.is-required")?.scrollIntoView?.({ behavior: "smooth", block: "center" }); return;
      }
      ...
      if (playerMode && /Class/i.test(currentStep) && classState.classId && !classChoiceStateComplete(classState)) {
        event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation?.(); const target = classChoiceStateRequiresSelection(classState) ? modal?.querySelector(".npc-forge-class-guide__subclasses.is-required") : modal?.querySelector(".npc-forge-class-guide"); target?.scrollIntoView?.({ behavior: "smooth", block: "center" });
      }
    }
  ...
  async function handleCreated(created) {
    ...
    Promise.race([persistSpeciesChoices(created, snapshot), new Promise((_, reject) => setTimeout(() => reject(new Error("species choice persistence timeout")), 5000))]).catch((error) => console.error("Could not persist species choices after character creation", error));
  }
  function resetChoiceState() { setSpeciesChoiceState({ speciesId: "", speciesName: "", rules: [], selections: {} }); setClassChoiceState({ ...EMPTY_CLASS_CHOICE_STATE, options: [] }); }

  return <NpcForgeSpeciesChoiceContext.Provider value={speciesContextValue}><NpcForgeClassChoiceContext.Provider value={classContextValue}><div className={playerMode ? "unified-player-character-forge" : undefined}><NewNpcModalV3Refined {...props} mode={playerMode ? "player" : "npc"} createCharacter={createCharacter} onReset={resetChoiceState} onCreated={handleCreated} /></div></NpcForgeClassChoiceContext.Provider><style jsx global>{`
