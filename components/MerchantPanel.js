...
/* NEW (fallback reroll needs these) */
import { loadItemsIndex, classifyUi } from "../utils/itemsIndex";

/* What each theme can sell (client-side fallback only) */
const THEME_RULES = {
  jeweler: (it, c) =>
  ...
  weapons: (it, c) => ["Melee Weapon", "Ranged Weapon", "Ammunition"].includes(c.uiType),

  alchemy: (it, c) => c.uiType === "Potions & Poisons",

  herbalist: (it, c) =>
    c.uiType === "Potions & Poisons" &&
    /\b(herb|salve|balm|elixir)\b/i.test((it.name || "") + " " + (it.item_description || "")),
  ...
  stable: (it, c) =>
    c.uiType === "Vehicles & Structures" || /\b(saddle|tack|bridle|harness)\b/i.test(it.name || ""),

  caravan: (it, c) =>
    c.uiType === "Adventuring Gear" ||
    /\b(tent|rations|pack|rope|wagon|cart)\b/i.test((it.name || "") + " " + (it.item_description || "")),
 ...
 * 4) NEW: rerollThemed() keeps your RPC first, and falls back to a client-side theme picker.
 */
export default function MerchantPanel({ merchant, isAdmin = false }) {
  ...
  function normalizeRow(row) {
    ...
    const name =
      row.display_name || payload.display_name || payload.item_name || payload.name || "Item";
    return {
      id: row.id,
  ...
  /* CHANGED: prefers your RPC; falls back to client-side themed selection */
  async function rerollThemed() {
    setBusyId("reroll");
    setErr("");
    try {
      // 1) Try server-side function first (keeps your DB-driven pipeline)
      ...
      if (res.error && /No function|does not exist/i.test(res.error.message)) {
        // 2) Fallback: client-side theme-based reroll using your items index
        await supabase.from("merchant_stock").delete().eq("merchant_id", merchant.id);

        const { byKey } = await loadItemsIndex(); // your catalog
        ...
        const picks = [];
        const used = new Set();
        while (picks.length < 16 && pool.length) {
          const i = Math.floor(Math.random() => Math.random() * pool.length); // fix lint: using Math.random twice is fine
        }
      } else if (res.error) {
        throw res.error;
      }
  ...
  return (
    <div className="container my-3 merchant-panel" id="merchantPanel">
      {/* removed inner id clash */}
        ...
        <div className="card mt-4">
          ...
          <div className="card-body">
            ...
            <div className="form-text mt-1">
              Strings are treated as single items. JSON with <code>qty</code>/<code>quantity</code>{" "}
              can be incremented. RPC <code>stock_merchant_item</code> will merge quantities when
              display names match.
            </div>
          </div>
        </div>
