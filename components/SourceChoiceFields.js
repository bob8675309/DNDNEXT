...
function prerequisiteText(option = {}) {
  ...
  if (typeof prerequisites === "object") return Object.entries(prerequisites).map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(", ") : String(value)}`).join(" • ");
  return "";
}
...
function RichField({ group, field, selected = [], blocked = [], onToggle }) {
  ...
  return <div className="npc-forge-rich-choice">
    <div className="npc-forge-rich-choice__head"><div><span>{field.label}</span><strong>{selected.length}/{count} selected</strong></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${field.label.toLowerCase()}…`} /></div>
    <div className="npc-forge-rich-choice__grid">
      ...
      <article className="npc-forge-rich-choice__detail">{focused ? <><header><div><span>{field.kind.replace(/-/g, " ")}</span><h4>{focused.label}</h4><small>{optionMeta(focused)}</small></div><button type="button" className={selectedSet.has(focused.key) ? "is-selected" : ""} onClick={() => onToggle?.(group.id, field.id, focused.key)} disabled={!selectedSet.has(focused.key) && complete}>{selectedSet.has(focused.key) ? "Remove" : complete ? "Selection complete" : "Choose"}</button></header>{prerequisiteText(focused) ? <div className="npc-forge-rich-choice__prerequisite"><strong>Prerequisite</strong><span>{prerequisiteText(focused)}</span></div> : null}<p>{focused.description || "This source-backed option has no additional imported description."}</p>{focused.metadata?.rangeText || focused.metadata?.damageDice ? <div className="npc-forge-rich-choice__facts">{focused.metadata?.rangeText ? <span>Range <b>{focused.metadata.rangeText}</b></span> : null}{focused.metadata?.damageDice ? <span>Damage <b>{focused.metadata.damageDice}{focused.metadata?.damageTypes?.length ? ` ${focused.metadata.damageTypes.join("/")}` : ""}</b></span> : null}</div> : null}</> : <p>Select an option from the catalogue.</p>}</article>
    </div>
  </div>;
}
