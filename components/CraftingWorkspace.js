...
function enchantingSlotProfileForRecipe(recipe = {}) {
...
function normalizeCraftingDisciplineLock(value) {
  const raw = String(value || "").trim().toLowerCase();
  if (!raw) return null;
...
export default function CraftingWorkspace({ mode = "page", disciplineLock = null, crafterId = null, crafter = null, isAdmin = false, startView = "recipes", showDisciplineSwitcher = true } = {}) {
  ...
  const knownCount = effectiveRecipes.filter((r) => r.known).length;
  ...
  const selectedKnownRecipe = selected && selected.known ? selected : effectiveRecipes.find((r) => r.known) || selected;
