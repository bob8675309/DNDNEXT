...
function SpeciesTraitDetails({ details = [], traits = [], choiceRules = [], selections = {}, onSelectChoice }) {
  ...
  return <div className="npc-forge-context-section npc-forge-species-features"><span>Species features</span><div className="npc-forge-species-feature-list">{described.map((entry, index) => {
    ...
    return <details key={`${entry.name}-${index}`} open={Boolean(rule)}><summary><span>{entry.name}</span>{rule ? <em className={complete ? "is-complete" : "is-required"}>{complete ? "Selected" : "Choose"}</em> : <em>Open</em>}</summary><p>{formatPlayerFacingText(entry.description)}</p>{rule ? <SpeciesTraitChoiceControl rule={rule} selections={selections} onSelect={onSelectChoice} /> : null}</details>;
...
export default function NpcForgeContextPanel({ step = 0, stepKey = "", detail = null, selectedSpecies = null, selectedBackground = null, backgroundMechanicDetails = null, selectedBackgroundFeat = null, backgroundFeatOptions = [], backgroundSkillSelections = {}, onToggleBackgroundSkill = null, onSelectBackgroundFeat = null, selectedClass = null, selectedSkill = null, selectedProfession = null, rolls = [], allocation = {}, finalAbilities = {}, draft = {} }) {
  ...
  const fallbacks = {
    ...
    training: ["Training", "Skills and professions", "Background grants and class choices combine here. Player expertise is not self-assigned."],
