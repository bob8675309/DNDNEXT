...
function InfoRows({ rows = [], wideDetails = false }) {
  return (
    <div className={`npc-forge-context-rows${wideDetails ? " npc-forge-background-info-rows" : ""}`}>
          ...
          <details key={row.label} className="npc-forge-context-row is-interactive" open={Boolean(row.defaultOpen)}>
            ...
            <div className="npc-forge-context-row-details">
              ...
              {details.map((entry, index) => {
                const body = <><div className={wideDetails ? "npc-forge-background-detail-head" : undefined}><b>{entry.label || row.label}</b>{entry.source ? <small>{sourceLabel(entry.source)}</small> : null}</div><p className={wideDetails ? "npc-forge-background-detail-copy" : undefined}>{formatPlayerFacingText(entry.description)}</p>{entry.prerequisite ? <small className={wideDetails ? "npc-forge-background-detail-prerequisite" : undefined}>Prerequisite: {formatPlayerFacingText(entry.prerequisite)}</small> : null}</>;
                return wideDetails
                  ? <div className="npc-forge-background-detail-card" key={`${entry.label || row.label}-${index}`}>{body}</div>
                  : <article key={`${entry.label || row.label}-${index}`}>{body}</article>;
              })}
            </div>
          </details>
        );
