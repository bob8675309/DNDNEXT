...
function selectedKeys(state, groupId, fieldId) {
  return Array.isArray(state?.selections?.[groupId]?.[fieldId]) ? state.selections[groupId][fieldId] : [];
}

function DropdownField({ group, field, selected, onSet }) {
  ...
  }}><option value="">Choose…</option>{(field.options || []).filter((option) => option.key === value || !slots.includes(option.key)).map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</select></label>)}</div>;
}

function ButtonField({ group, field, selected, onToggle }) {
  return <div className="npc-forge-source-choice-buttons">{(field.options || []).map((option) => <button key={option.key} type="button" className={selected.includes(option.key) ? "is-selected" : ""} onClick={() => onToggle(group.id, field.id, option.key)}><strong>{option.label}</strong>{option.description ? <small>{option.description}</small> : null}</button>)}</div>;
}
...
export default function NpcForgeSourceChoiceFields({ placement, ownerType = "", title = "Required source choices", empty = null }) {
  ...
  return <div className="npc-forge-source-choices"><div className="npc-forge-source-choices__heading"><span>Source choices</span><strong>{title}</strong></div>{groups.map((group) => {
    const complete = (group.fields || []).every((field) => sourceChoiceFieldComplete(group, field, state.selections));
    return <section key={group.id} className={`npc-forge-source-choice-group ${complete ? "is-complete" : "is-required"}`}><header><div><strong>{group.label}</strong><small>{group.source || "Campaign"}{group.helper ? ` • ${group.helper}` : ""}</small></div><em>{complete ? "Complete" : "Required"}</em></header>{(group.fields || []).map((field) => {
      const selected = selectedKeys(state, group.id, field.id);
      const useDropdowns = ["tool", "language", "spell", "item", "weapon"].includes(field.kind) || (field.options || []).length > 8 || Number(field.count || 1) > 1;
      return <div key={field.id} className="npc-forge-source-choice-field">{useDropdowns ? <DropdownField group={group} field={field} selected={selected} onSet={setChoice} /> : <><span>{field.label}</span><ButtonField group={group} field={field} selected={selected} onToggle={toggleChoice} /></>}{field.replacementCadence ? <small className="npc-forge-source-choice-note">May be replaced: {String(field.replacementCadence).replace(/-/g, " ")}.</small> : null}</div>;
    })}</section>;
  })}<style jsx global>{`
