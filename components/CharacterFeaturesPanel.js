...
export default function CharacterFeaturesPanel({ character = null, isAdmin = false }) {
  ...
  const loadData = useCallback(async ({ preserveNotice = false } = {}) => {
    ...
    if (grantsResult.error) setError(grantsResult.error.message || "Could not load granted features.");
  ...
  async function grantSelected() {
    ...
    if (grantError) {
      setError(grantError.message || "Could not grant this feature.");
    } else {
  ...
  async function removeGrant(grant) {
    ...
    if (removeError) {
      setError(removeError.message || "Could not remove this feature.");
    } else {
  ...
  return (
    <div className="character-features-panel">
      <div className="npc-card mb-3 feature-summary">
        <div>
          <div className="spell-admin-kicker">Features & Boons</div>
          <h2 className="h5 mb-1">{character?.name || "Character"}</h2>
          <div className="small text-muted">Origin, class, campaign, and Game Master-granted features are kept distinct.</div>
        </div>
        ...
        <div className="col-12 col-xl-7">
          <section className="npc-card">
            <div className="d-flex align-items-start justify-content-between gap-2 flex-wrap mb-3">
              <div>
                <div className="npc-card-title mb-0">{isAdmin ? "Grant a Feature" : "Feature Details"}</div>
                <div className="small text-muted">The preferred source version is shown when names repeat.</div>
              </div>
            ...
            <div className="feature-catalog-list mb-3">
      ...
      <style jsx>{`
        ...
        .feature-catalog-list { display:grid; gap:.4rem; max-height:32vh; overflow:auto; }
