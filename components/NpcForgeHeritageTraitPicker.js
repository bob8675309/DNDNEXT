...
export default function NpcForgeHeritageTraitPicker({ group, selections = {}, onSet = null }) {
  ...
  return <section className={`npc-forge-heritage-picker ${picks.length === maxPicks ? "is-complete" : "is-required"}`}>
    ...
    {selectedOptions.length ? <div className="npc-forge-heritage-picker__selected"><div>{selectedOptions.map(({ option, count }) => <span key={option.key} className={`npc-forge-heritage-picker__selected-chip ${focused?.key === option.key ? "is-focused" : ""}`}><button type="button" className="npc-forge-heritage-picker__selected-focus" onClick={() => setFocusedKey(option.key)}><strong>{option.label}</strong>{count > 1 ? <small>{count} picks</small> : null}</button><button type="button" className="npc-forge-heritage-picker__selected-remove" aria-label={`Remove one ${option.label} Heritage Trait pick`} title={`Remove one ${option.label} pick`} onClick={() => removeTrait(option.key)}>×</button></span>)}</div></div> : null}
