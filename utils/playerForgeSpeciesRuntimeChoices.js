...
function eladrinSeasonOptions() {
  return [
    { key: "autumn", value: "autumn", label: "Autumn", description: "Peace and goodwill. Starting at character level 3, after you use Fey Step, up to two creatures of your choice that you can see within 10 feet of you must succeed on a Wisdom saving throw or be Charmed by you for 1 minute. The effect ends early if you or your companions deal damage to that creature.", source: "MPMM", kind: "season", metadata: { feyStepEffect: "charm", sourceItem: { name: "Autumn" } } },
    { key: "winter", value: "winter", label: "Winter", description: "Sorrow and dread. Starting at character level 3, immediately before you use Fey Step, one creature of your choice that you can see within 5 feet of you must succeed on a Wisdom saving throw or be Frightened of you until the end of your next turn.", source: "MPMM", kind: "season", metadata: { feyStepEffect: "frighten", sourceItem: { name: "Winter" } } },
    { key: "spring", value: "spring", label: "Spring", description: "Joy and renewal. Starting at character level 3, when you use Fey Step, you can touch one willing creature within 5 feet of you. That creature teleports instead of you, appearing in an unoccupied space of your choice that you can see within 30 feet of you.", source: "MPMM", kind: "season", metadata: { feyStepEffect: "ally-teleport", sourceItem: { name: "Spring" } } },
    { key: "summer", value: "summer", label: "Summer", description: "Boldness and fury. Starting at character level 3, immediately after you use Fey Step, each creature of your choice that you can see within 5 feet of you takes Fire damage equal to your Proficiency Bonus.", source: "MPMM", kind: "season", metadata: { feyStepEffect: "fire-damage", sourceItem: { name: "Summer" } } },
  ];
}
...
export function applySpeciesRuntimeChoiceAuthority({ groups = [], species = null, toolRows = [] } = {}) {
  ...
  if (identity.name === "eladrin" && identity.source === "MPMM") {
    filtered.push({
      ...
      placement: "species",
      level: 1,
      helper: "Choose your starting season. Starting at character level 3, the selected season adds the listed effect to Fey Step.",
      fields: [{
        id: "season",
