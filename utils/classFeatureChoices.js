...
export function buildClassFeatureChoiceGroups({ selectedClass, level = 1, features = [], selectedSubclass = null, catalogRows = [], items = [] } = {}) {
  ...
  rows.forEach((row) => {
    ...
    nodes.forEach((node, index) => {
      ...
      const references = referencedOptions(node.entries || node).map((option) => ({ ...option, source: option.source || row.source || row.subclass_source || row.class_source || selectedClass.source }));
    ...
    if (!addedChoiceGroup && permanentChoiceText(row)) {
      const directOptions = namedEntryOptions(row.entries).map((option) => ({ ...option, source: option.source || row.source || row.subclass_source || row.class_source || selectedClass.source }));
  ...
  const existingKinds = new Set(merged.map((group) => group.kind));
  const custom = customGroups(rows, selectedClass, level, catalogRows, items, features).filter((group) => !existingKinds.has(group.kind));
  return [...merged, ...custom].filter((group) => group.count > 0 && group.options.length >= group.count).sort((a, b) => Number(a.level) - Number(b.level) || a.label.localeCompare(b.label));
}
...
export function classFeatureGroupsComplete(groups = [], selections = {}) { return groups.every((group) => !group.required || unique(selections?.[group.id] || []).length === Number(group.count || 0)); }
export function toggleClassFeatureSelection(groups = [], selections = {}, groupId, optionKey) {
  ...
  if (!exists) {
    const allSelected = groups.flatMap((candidate) => (selections?.[candidate.id] || []).map((key) => candidate.options.find((entry) => entry.key === key))).filter(Boolean);
...
export function selectedClassFeatureOptions(groups = [], selections = {}) {
  return groups.flatMap((group) => (selections?.[group.id] || []).map((key) => {
    const option = group.options.find((candidate) => candidate.key === key);
    return option ? { groupId: group.id, groupLabel: group.label, groupKind: group.kind, level: group.level, key: option.key, name: option.name, source: option.source, kind: option.kind, description: option.description, minLevel: option.minLevel, requires: option.requires || "", followup: option.followup || "" } : null;
  }).filter(Boolean));
}
export function serializeClassFeatureChoices(groups = [], selections = {}) {
  return Object.fromEntries(groups.map((group) => [group.id, {
    label: group.label, kind: group.kind, level: group.level, count: group.count, sourceFeature: group.sourceFeature || group.label, subclassName: group.subclassName || null,
    allowRepeatAcrossGroups: Boolean(group.allowRepeatAcrossGroups),
    selections: (selections?.[group.id] || []).map((key) => {
      const option = group.options.find((candidate) => candidate.key === key);
      return option ? { key: option.key, name: option.name, source: option.source, kind: option.kind, followup: option.followup || null } : null;
    }).filter(Boolean),
  }]));
}
