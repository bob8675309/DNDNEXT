...
- [`Character_Sheet_Identity_Reconciliation.md`](./Character_Sheet_Identity_Reconciliation.md) — **required reading before changing NPC selection, asynchronous sheet/equipment loading, tab visibility refresh, or `CharacterSheetPanel` synchronization.** Defines the character/request identity guard, loading boundary, edit behavior, and late-response rejection contract.
