...
- `components/CharacterSheet5e.js` owns the compact standalone weapon/cantrip/prepared-spell/feature-action surface. `utils/characterSheetActions.js` keeps one row per physical weapon, exposes a selected melee/thrown mode profile for dual-use weapons, applies conditional species reach, and returns a rollable damage formula beside attack math; `utils/characterSheetFeatures.js` combines unique Feats & Boons grants with acquired species/base-class/subclass features. `sql/20260802_02_character_sheet_feature_actions.sql` owns guarded persistent standalone feature state beginning with Rage. `scripts/validate_player_sheet_actions.mjs` guards feat deduplication, feature-level boundaries, inventory permission RPCs, selected-mode and simultaneous attack/damage math, profile auto-open, and the boundary that leaves encounter execution server-authoritative.
