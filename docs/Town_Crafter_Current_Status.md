...
Last updated after green deployment: `2b1386783b36b3d7eef62bd9e15464b08d3dab98`.
...
- The wrapper delegates to `NpcPanel`, normalizes the future shared view names, resolves craft capability through `utils/craftProfession.js`, and now owns safe wrapper-level interaction view state.
- The wrapper passes `craftProfession`, `hasCraftCapability`, `interactionView`, and `setInteractionView` forward for later use, but the visible panel still does not expose Craft.
...
3. Add a wrapper-hosted craft view shell without rendering `CraftingWorkspace` yet.
