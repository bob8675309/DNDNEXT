...
The historical defect had three independent causes:
...
1. updates the active identity reference;
2. invalidates all outstanding sheet/equipment/notes request IDs;
3. clears the previous sheet and controlled draft;
4. closes sheet edit mode;
5. clears equipped rows and notes;
6. clears the prior roll result;
7. enters a bounded loading state;
8. commits the new `selectedKey`.
...
No special focus handler is required. A background tab may delay a request, but the request/identity guards reject it if a newer selection has occurred. Returning to the tab must not make an old response current again.
