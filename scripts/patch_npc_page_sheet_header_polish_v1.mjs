...
// This is intentionally robust even if older patch scripts did not add profile
// props successfully during build.
// -----------------------------------------------------------------------------
{
  ...
  if (!source.includes('className="btn btn-sm btn-outline-info me-2"')) {
    source = replaceOnce(
      source,
...
// -----------------------------------------------------------------------------
// NPC page: sheet header sprite thumbnail, double-click sprite picker, description
// label alignment, remove Character sheet label, and in-place Profile button.
// This script runs after the earlier NPC page/portrait patch scripts.
// -----------------------------------------------------------------------------
{
  ...
  if (!source.includes("async function applySpritePatchToSelected")) {
    source = replaceOnce(
      ...
      `  async function applySpritePatchToSelected(nextIconId) {\n    if (!selected?.id) return;\n    const prev = selected?.map_icon_id || null;\n    const idStr = String(selected.id);\n    const patchLocal = (val) => {\n      setNpcs((arr) =>\n        (arr || []).map((c) =>\n          String(c.id) === idStr ? { ...c, map_icon_id: val } : c\n        )\n      );\n      setMerchants((arr) =>\n        (arr || []).map((c) =>\n          String(c.id) === idStr ? { ...c, map_icon_id: val } : c\n        )\n      );\n    };\n\n    patchLocal(nextIconId || null);\n    const upd = await supabase\n      .from("characters")\n      .update({ map_icon_id: nextIconId || null, updated_at: new Date().toISOString() })\n      .eq("id", selected.id);\n\n    if (upd.error) {\n      console.error(upd.error);\n      patchLocal(prev);\n      alert(upd.error.message || "Failed to save sprite");\n      return;\n    }\n\n    await Promise.all([loadNpcs(), loadMerchants(), loadMerchantProfiles()]);\n  }\n\n  function applyPortraitPatchToSelected(patch) {`,
      "NPC page sprite patch helper"
    );
  }
  ...
  source = replaceOnce(
    ...
    '                       profileHref={selected?.id ? `/map?npc=${encodeURIComponent(selected.id)}` : null}\n                       profileText="Profile"\n                       onOpenProfile={() => setProfilePanelOpen(true)}\n                       inventoryHref={inventoryHref || null}',
    "NPC page profile prop idempotent"
  );

  // If the prior patch did not add the in-place profile prop, add it now beside inventory.
  source = replaceOnce(
    source,
