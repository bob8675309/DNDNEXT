...
function crafterKnownKeysFrom(character = {}) {
  const out = [];
  const direct = character?.known_recipe_ids || character?.knownRecipeIds || character?.craft_known_recipe_ids || character?.craftKnownRecipeIds;
  if (Array.isArray(direct)) out.push(...direct.map((value) => String(value || "").trim().toLowerCase()).filter(Boolean));
  const tags = Array.isArray(character?.tags) ? character.tags : [];
  tags.forEach((tagValue) => {
    const tagText = String(tagValue || "").trim();
    if (tagText.toLowerCase().startsWith("known_recipe:")) out.push(tagText.slice("known_recipe:".length).trim().toLowerCase());
  });
...
replaceRequired(
  ...
  useEffect(() => {
    if (!isNpcCrafterPanel) return;
    setCrafterKnownRecipeIds(new Set(crafterKnownKeysFrom(crafter)));
    setCrafterKnownSaveState("");
  }, [isNpcCrafterPanel, crafterId, crafterTagsKey]);
  ...
  async function toggleCrafterKnownRecipe(recipe, nextKnown) {
    if (!isAdmin || !isNpcCrafterPanel || !recipe) return;
    ...
    try {
      const baseTags = (Array.isArray(crafter?.tags) ? crafter.tags : []).filter((tagValue) => !String(tagValue || "").toLowerCase().startsWith("known_recipe:"));
      const knownTags = Array.from(next).sort().map((knownKey) => `known_recipe:${knownKey}`);
      const { error } = await supabase.from("characters").update({ tags: [...baseTags, ...knownTags], updated_at: new Date().toISOString() }).eq("id", crafterId);
  ...
  "CraftingWorkspace crafter known toggle helpers"
);
...
for (const token of ["isAdmin = false", "function crafterKnownKeysFrom", "function RecipeSortHeader", "canEditKnown={isAdmin && isNpcCrafterPanel}", "onToggleKnown={toggleCrafterKnownRecipe}", "craft-page-panel-mode", "panelCraftTabs", "known_recipe:", "crafterGate"]) {
  requireToken(token, "NPC crafter panel recipe UI patch");
}
requireAbsent('TABS.map(([id, icon, label]) => <button key={id} type="button" className={cls("craft-tab", activeTab === id && "craft-tab-active")} onClick={() => setActiveTab(id)}', "NPC crafter panel tab filter should replace unfiltered tabbar");
console.log("Patched NPC crafter panel recipe UI.");
