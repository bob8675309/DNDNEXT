...
function patchCharacterSheet() {
  ...
  if (!source.includes('from "../utils/craftingProfessions"')) {
    source = replaceOnce(
      source,
...
function patchItemsPage() {
  ...
  if (!source.includes("buildCrafterProfessionSnapshot")) {
    source = replaceOnce(
      source,
    ...
    source = replaceOnce(
      ...
      "Profession-aware alchemy preview"
    );
    ...
    source = replaceOnce(
      ...
      "Profession check ability defaults"
    );
    ...
    source = replaceOnce(
      ...
      "Profession check tool defaults"
    );
    ...
    source = replaceOnce(
      ...
      '    const key = `${requested}::${shouldCraft}::${router.query.crafter || ""}::${router.query.town || ""}`;',
      "Workshop query idempotence key"
    );
    ...
    source = replaceOnce(
      ...
      '  }, [router.isReady, router.query.discipline, router.query.craft, router.query.crafter, router.query.town, recipes]);',
      "Workshop query town dependency"
    );
...
function patchTownSheet() {
  ...
  if (!source.includes("disciplineForCraftType")) {
    source = replaceOnce(
      source,
      lines(
    ...
    source = replaceOnce(
      ...
      "Town crafter direct canonical links"
    );
    ...
    source = replaceOnce(
      ...
      "Town profession scope"
    );

    fs.writeFileSync(target, source, "utf8");
  }

  for (const token of ["disciplineForCraftType", "Open {entry.discipline}", 'pathname: "/items"']) {
    if (!source.includes(token)) throw new Error(`TownSheet validation failed: ${token}`);
  }
}
...
function patchStyles() {
  ...
  if (!source.includes(marker)) {
    source += "\n\n" + lines(
      marker,
      '.csheet-profession-row { align-items: flex-start; }',
      '.csheet-profession-main { flex: 1 1 auto; min-width: 0; }',
      '.csheet-profession-ability { margin-top: .35rem; min-height: 30px; font-size: .76rem; }',
      '.csheet-profession-tool, .csheet-profession-note { color: rgba(255,255,255,.64); font-size: .72rem; line-height: 1.35; }',
      '.csheet-profession-tool { padding: .2rem .35rem 0; }',
      '.csheet-profession-note { margin-top: .65rem; padding: .55rem .65rem; border: 1px solid rgba(169,137,220,.24); border-radius: 8px; background: rgba(31,23,45,.58); }',
      '.craft-provider-layout { position: relative; }',
      '.craft-provider-layout::before { content: ""; position: absolute; inset: -14px; pointer-events: none; border-radius: 22px; background: radial-gradient(circle at 12% 5%, rgba(236,185,92,.12), transparent 34%), linear-gradient(135deg, rgba(77,49,28,.2), transparent 46%); }',
      '.craft-provider-layout > * { position: relative; z-index: 1; }',
      '.craft-provider-card { border-color: rgba(233,184,93,.58); background: linear-gradient(135deg, rgba(76,48,27,.72), rgba(24,20,31,.94)), radial-gradient(circle at top right, rgba(220,158,63,.18), transparent 42%); box-shadow: inset 0 0 0 1px rgba(255,224,160,.06), 0 16px 42px rgba(0,0,0,.24); }',
      '.craft-provider-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; }',
      '.craft-provider-head h3 { margin: .2rem 0 .25rem; }',
      '.craft-provider-head p { margin: 0; }',
      '.craft-provider-grid { display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); gap: .55rem; margin-top: .85rem; }',
      '.craft-provider-grid > div { display: flex; flex-direction: column; gap: .15rem; padding: .6rem .7rem; border: 1px solid rgba(255,224,160,.16); border-radius: 9px; background: rgba(12,10,16,.42); }',
      '.craft-provider-grid span { color: rgba(255,255,255,.58); font-size: .7rem; text-transform: uppercase; letter-spacing: .06em; }',
      '@media (max-width: 900px) { .craft-provider-grid { grid-template-columns: repeat(2,minmax(0,1fr)); } }',
      '@media (max-width: 560px) { .craft-provider-head { display: block; } .craft-provider-head .craft-status-pill { display: inline-flex; margin-top: .65rem; } .craft-provider-grid { grid-template-columns: 1fr; } }'
    ) + "\n";
    fs.writeFileSync(target, source, "utf8");
  }
