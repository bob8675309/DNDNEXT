...
for (const token of [
  ...
  "Render 32 frames and assemble atlas", "dndnext_dawn_model_builder.py",
  "dndnext_dawn_visual_refinement_v3.py", "dndnext_dawn_prepare_scene.py",
  "dndnext_sprite_export.py", "dndnext_sprite_export_runner.py",
  ...
  assert(windowsBuild.includes(token), `Windows Dawn v3 pipeline is missing ${token}`);
}
assert(!windowsBuild.includes("Normalize Dawn diagonal baseline"), "Dawn v3 must not run pose baseline correction");
assert(!windowsBuild.includes("Normalize bounded rendered baseline"), "Dawn v3 must not shift rendered PNG frames");
assert(!windowsBuild.includes("HandledExitCodes @(2)"), "Dawn v3 QA failures must stop the build");
...
for (const source of [exporter, exporterRunner, setup, builder, refinement, prepare]) {
...
console.log("Blender sprite export and Dawn v3 source-only animation validation passed.");
