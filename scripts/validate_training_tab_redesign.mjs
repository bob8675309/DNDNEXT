...
assert(tabbedTraining.includes("Mundane tool proficiency is separate from Trade Skill training") && tabbedTraining.includes("neither one automatically grants the other"), "Skills navigation must state the new tool/Trade Skill separation rule.");
assert(!tabbedTraining.includes("sourceGrantedTradeSkills") && !tabbedTraining.includes("professionKeysForTools"), "Skills/Feats completion must not count source-granted mundane tools as free Trade Skills.");
...
assert(playerTraining.includes("Granted by Background") && playerTraining.includes("Granted by ${grantSource}"), "Inline granted-skill provenance is missing.");
assert(playerTraining.includes("Trade Skills measure crafting proficiency") && playerTraining.includes("gaining a normal tool or tool proficiency does not grant this Trade Skill or Expertise"), "Player Training must explain that mundane tools and Trade Skill rank are separate.");
assert(!playerTraining.includes("sourceGrantedProfessionKeys") && !playerTraining.includes("sourceTradeFields") && !playerTraining.includes("professionGrantSource"), "Player Training must not project source-granted tools into Trade Skill ranks.");
assert(!playerTraining.includes("Source-granted craft tools resolve here for free") && !playerTraining.includes("A mapped crafting tool and its Trade Skill are one proficiency"), "Rejected tool-to-Trade-Skill coupling remains in player Training.");
assert(playerTraining.includes('field.kind === "skill-or-tool"'), "Mixed skill-or-tool source fields must remain representable so skill results can flow into Skills while tool results remain source choices.");
assert(playerTraining.includes("groupsOverride={genericSourceTrainingGroups}"), "Generic non-feat source chooser must use a bounded presentation override after inline skill routing.");
assert(playerTraining.includes('if (field.kind === "skill") return []') && playerTraining.includes('filter((option) => !skillKeyForOption(option))'), "Tool options must remain in source Training after skill options are promoted into the Skills list.");
...
assert(playerTraining.includes("explicitlyTrainedProfessionKeys.length") && playerTraining.includes("usedTrainingChoices = selectedClassSkills.length + paidProfessionCount"), "Every explicitly trained Trade Skill must consume the shared allowance regardless of mundane tool proficiency.");
...
assert(routedController.includes("Mundane tool proficiency is separate") && !routedController.includes("toolForProfession") && !routedController.includes("professionKeysForTools") && !routedController.includes("additionalTools"), "Routed controller must not auto-grant tools from Trade Skill training or discount source tools as free Trade Skills.");
...
assert(trainingContext.includes("associated mundane tool and is handled separately") && trainingContext.includes("does not grant this Trade Skill or Expertise") && trainingContext.includes("normally required"), "Trade Skill dossier must explain mundane tool separation and requirement.");
assert(!trainingContext.includes("proficiency and the ${selectedProfession.label} Trade Skill are the same campaign proficiency"), "Rejected tool/Trade Skill equivalence remains in Current Selection.");
...
assert(craftingToolProfessions.includes("TRADE_SKILL_KEYS") && craftingToolProfessions.includes("professionKeyForTool") && craftingToolProfessions.includes("professionKeysForTools"), "Canonical tool-to-Trade-Skill association helper must derive from all player Trade Skill definitions.");
assert(!craftingToolProfessions.includes("mergeToolGrantedProfessions") && craftingToolProfessions.includes("does not grant a Trade Skill rank"), "Tool association helpers must not promote source-granted tools into persisted Trade Skill rank.");
...
assert(craftingProfessions.includes("hasToolProficiency") && craftingProfessions.includes("toolRequiredForCrafting: true") && !craftingProfessions.includes("toolGranted ? 1 : 0"), "Shared crafting resolver must expose mundane-tool status separately without granting proficiency.");
assert(craftingProfessions.includes("professionHasServiceFlag") && craftingProfessions.includes("professionServicesFromSheet"), "Tool separation must not remove explicit NPC workshop service authority.");
...
console.log("Training tab redesign validation passed: Skills/Feats internal views, 40/60 choice-detail layout, categorical completion guidance, separate mundane tools and Trade Skill ranks, Crafter's bounded Profession choices in Skills, other feat-owned non-spell decisions in Current Selection, mixed feat spell routing to Spells, isolated NPC fallback, eight player Trade Skills with four-discipline runtime isolation, compact Species languages, header geometry reset, sticky Current Selection, and source-owned completion are intact.");
