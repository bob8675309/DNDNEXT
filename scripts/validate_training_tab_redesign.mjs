...
assert(sourceContext.includes('projectedPlacement = placement === "training"') && sourceContext.includes('String(group.ownerType || "") === "feat"') && sourceContext.includes('? "class"'), "Every feat-owned non-spell projected Training group must surface through the Feats subsection, including proficiency feats.");
...
for (const token of ["Current Selection", "Typical Uses", "Class Skill", "Trade Skill", "Associated Tool", "Feat Rules", "Required Feat Choices", "Every permanent non-spell decision owned by this feat", "Granted spells resolve on the next tab", "groupsOverride={trainingGroups}", "All choices can be reviewed on the final step", "Cooking", "Tinkering", "Jewelcraft", "Brewing", "Proficiency now • recipes later"]) assert(trainingContext.includes(token), `Training context dossier missing ${token}`);
assert(!trainingContext.includes("proficiencyGroups") && !trainingContext.includes("Training proficiencies resolve under Skills"), "Feat-owned skill/tool/instrument decisions must not keep a separate Skills-side chooser; only their resolved grants may appear in Skills.");
...
console.log("Training tab redesign validation passed: Skills/Feats internal views, 40/60 choice-detail layout, categorical completion guidance, all feat-owned non-spell decisions in right-side Current Selection, mixed feat spell routing to Spells, isolated NPC fallback, inline non-feat source grants, eight player Trade Skills with mapped tool persistence and four-discipline runtime isolation, compact Species languages, header geometry reset, sticky Current Selection, and source-owned completion are intact.");
