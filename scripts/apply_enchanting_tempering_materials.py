...
TARGET = Path("pages/items.js")
BRANCH = "automation/enchanting-tempering-materials-run"

if "const ENCHANTING_SECTIONS" not in TARGET.read_text():
    ...
    subprocess.run(["git", "config", "user.name", "DNDNext Patch Bot"], check=True)
    subprocess.run(["git", "config", "user.email", "actions@users.noreply.github.com"], check=True)
    subprocess.run(["git", "add", "pages/items.js"], check=True)
    changed = subprocess.run(["git", "diff", "--cached", "--quiet"]).returncode != 0
    if changed:
        subprocess.run(["git", "commit", "-m", "feat: generate enchanting and tempering source"], check=True)
        subprocess.run(["git", "push", "origin", f"HEAD:{BRANCH}"], check=True)
else:
    print("Crafting source is already patched; continuing to validation.")
