...
for (const source of [migration, normalize, panel, currency]) {
  for (const token of ["MapPageClient", "map_routes", "advance_all_characters", "player_wallets"]) forbid(source, token);
}
for (const token of ["insert into public.inventory_items", "update public.encounter_participants", "insert into public.encounter_participants"]) forbid(migration, token);

console.log("Boon of Energy Resistance per-feat-instance acquisition pair, Long-Rest replacement, runtime projection, ACLs, and protected boundaries validated.");
