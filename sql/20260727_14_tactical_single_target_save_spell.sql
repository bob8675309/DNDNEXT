...
create or replace function public.encounter_cast_spell_v2(
  p_caster_id uuid,
...
declare
  ...
  v_roll integer;
  v_total integer;
  v_save_advantage boolean:=false;
...
begin
  ...
  if v_key in ('fire-bolt|xphb','cure-wounds|xphb') then
    return public.encounter_cast_spell_v1(
      p_caster_id,
      p_assignment_id,
      p_target_id,
      p_slot_level,
  ...
  if v_key<>'sacred-flame|xphb' then
  ...
  if v_spell.level<>0 then
    raise exception 'Sacred Flame must resolve from its reviewed cantrip definition';
  end if;
  ...
  if v_dist_ft>60 then raise exception 'Target is beyond Sacred Flame range'; end if;
  ...
  v_roll:=case when v_save_advantage then greatest(v_roll1,v_roll2) else v_roll1 end;
  v_total:=v_roll+v_save_bonus;
  v_save_success:=v_total>=v_save_dc;
  ...
  if not v_save_success then
    ...
    v_damage:=public.encounter_apply_damage_internal_v1(v_t.id,v_raw_damage,'radiant');
  else
  ...
  v_result:=jsonb_build_object(
    ...
    'saveRoll',v_roll,
    'secondRoll',case when v_save_advantage then v_roll2 else null end,
    'saveAdvantage',v_save_advantage,
    'saveTotal',v_total,
  ...
  insert into public.encounter_combat_log(
    ...
    v_c.display_name||' cast Sacred Flame on '||v_t.display_name||'; DEX save '||
      case when v_save_success then 'succeeded.' else 'failed.' end,
    v_result
  );
...
revoke all on function public.encounter_cast_spell_v2(uuid,uuid,uuid,integer,uuid) from public, anon;
grant execute on function public.encounter_cast_spell_v2(uuid,uuid,uuid,integer,uuid) to authenticated, service_role;

do $postconditions$
begin
  if not has_function_privilege('authenticated','public.encounter_cast_spell_v2(uuid,uuid,uuid,integer,uuid)','EXECUTE') then
    raise exception 'guarded save-spell cast RPC missing';
  end if;
  if has_function_privilege('anon','public.encounter_cast_spell_v2(uuid,uuid,uuid,integer,uuid)','EXECUTE') then
    raise exception 'anon must not cast save spells';
  end if;
