...
create or replace function public.encounter_cast_spell_v3(
  p_caster_id uuid,
...
declare
  ...
  v_target_wounded boolean:=false;
  v_i integer;
...
begin
  ...
  if v_key in ('fire-bolt|xphb','cure-wounds|xphb','sacred-flame|xphb') then
    return public.encounter_cast_spell_v2(
      p_caster_id,
      p_assignment_id,
      p_target_id,
      p_slot_level,
  ...
  if v_key<>'toll-the-dead|xphb' then
  ...
  if v_spell.level<>0 then
    raise exception 'Toll the Dead must resolve from its reviewed cantrip definition';
  end if;
  ...
  if v_dist_ft>60 then raise exception 'Target is beyond Toll the Dead range'; end if;
  ...
  insert into public.encounter_combat_log(
    ...
    v_c.display_name||' cast Toll the Dead on '||v_t.display_name||'; WIS save '||
...
revoke all on function public.encounter_cast_spell_v3(uuid,uuid,uuid,integer,uuid) from public, anon;
grant execute on function public.encounter_cast_spell_v3(uuid,uuid,uuid,integer,uuid) to authenticated, service_role;

do $postconditions$
begin
  if not has_function_privilege('authenticated','public.encounter_cast_spell_v3(uuid,uuid,uuid,integer,uuid)','EXECUTE') then
    raise exception 'guarded Toll the Dead cast RPC missing';
  end if;
  if has_function_privilege('anon','public.encounter_cast_spell_v3(uuid,uuid,uuid,integer,uuid)','EXECUTE') then
    raise exception 'anon must not cast Toll the Dead';
  end if;
