...
alter table public.encounter_command_requests
  ...
  check (command_type in ('move','end_turn','core_action','attack','weapon_attack','save','reaction','effect','spell_cast'));

alter table public.encounter_spell_slots replica identity full;
do $realtime$
begin
  if not exists (
    select 1 from pg_publication_tables
    where pubname='supabase_realtime' and schemaname='public' and tablename='encounter_spell_slots'
  ) then
    alter publication supabase_realtime add table public.encounter_spell_slots;
  end if;
end;
$realtime$;
...
begin
  ...
  if v_key not in ('fire-bolt|xphb','cure-wounds|xphb') then
    raise exception 'This spell remains GM-assisted; no automated tactical adapter is approved yet';
  end if;
  if v_spell.level>0 and not (v_assignment.prepared or v_assignment.always_available) then
  ...
  if v_key='fire-bolt|xphb' then
    if p_target_id=v_c.id then raise exception 'Fire Bolt requires another creature target in this automation slice'; end if;
    if v_t.is_defeated then raise exception 'Target is already defeated'; end if;
    if v_dist_ft>120 then raise exception 'Target is beyond Fire Bolt range'; end if;
    if exists (
    ...
    if v_hit then
      ...
      v_damage:=jsonb_build_object(
        'targetId',v_t.id,'damageType','fire','rawDamage',0,
  ...
  elsif v_key='cure-wounds|xphb' then
    ...
    v_result:=jsonb_build_object(
      ...
      'castingAbility',v_casting_ability,
      'castingAbilityModifier',v_cast_mod,
      'healingDice',v_dice_count::text||'d'||v_die_size::text,
      'healingRoll',v_heal_roll,
      'healingRequested',v_heal_total,
      'healing',v_healing,
      'targeting',v_targeting
    );
  end if;
  ...
  insert into public.encounter_combat_log(
    ...
    v_c.display_name||' cast '||v_spell.name||' on '||v_t.display_name||'.',v_result
  );
...
begin
  if not has_function_privilege('authenticated','public.encounter_cast_spell_v1(uuid,uuid,uuid,integer,uuid)','EXECUTE') then
    raise exception 'guarded spell cast RPC missing';
  end if;
  if has_function_privilege('anon','public.encounter_cast_spell_v1(uuid,uuid,uuid,integer,uuid)','EXECUTE') then
    raise exception 'anon must not cast spells';
  end if;
  if not exists (
    select 1 from pg_publication_tables
    where pubname='supabase_realtime' and schemaname='public' and tablename='encounter_spell_slots'
  ) then
    raise exception 'encounter spell slots must be in realtime publication';
  end if;
end;
$postconditions$;
