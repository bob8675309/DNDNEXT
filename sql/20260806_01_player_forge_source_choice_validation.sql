-- Deferred Player Forge source-choice validation.
-- Validated permanent group kinds include eldritch-invocation, battle-master-maneuver,
-- weapon-mastery, fighting-style, expertise, artificer-plan, arcane-shot, rune,
-- elemental-discipline, kensei-weapon, and source-backed class-feature choices.
...
begin
  ...
  if not v_legacy then
    if jsonb_typeof(coalesce(v_sheet -> 'speciesTraitChoices', '{}'::jsonb)) <> 'object'
       ...
       or jsonb_typeof(coalesce(v_sheet -> 'classChoiceFeats', '[]'::jsonb)) <> 'array' then
    ...
    if v_species_key = 'human' then
      ...
      if not exists (
        ...
        where o.option_type = 'skill'
          and private.normalize_player_choice_name_v1(o.option_key) =
              private.normalize_player_choice_name_v1(v_sheet #>> '{speciesTraitChoices,skillful,skill}')
      ) then
      ...
      if not exists (
        ...
        where o.option_type = 'feat'
          and o.category = 'O'
          and private.normalize_player_choice_name_v1(o.name) =
              private.normalize_player_choice_name_v1(v_sheet #>> '{speciesTraitChoices,versatile,feat}')
      ) then
      ...
      if not exists (
        select 1
        from jsonb_array_elements(coalesce(v_sheet -> 'speciesSkillChoices', '[]'::jsonb)) choice
        where private.normalize_player_choice_name_v1(choice ->> 'value') =
              private.normalize_player_choice_name_v1(v_sheet #>> '{speciesTraitChoices,skillful,skill}')
      ) then
      ...
      if not exists (
        ...
        where private.normalize_player_choice_name_v1(coalesce(choice ->> 'label', choice ->> 'name')) =
              private.normalize_player_choice_name_v1(v_sheet #>> '{speciesTraitChoices,versatile,feat}')
      ) then
    ...
    loop
      if jsonb_typeof(v_group) <> 'object'
         or jsonb_typeof(coalesce(v_group -> 'selections', '[]'::jsonb)) <> 'array' then
      ...
      for v_selection in
        select value from jsonb_array_elements(v_group -> 'selections')
      loop
        if not private.player_forge_choice_option_is_valid_v1(
          new.class_id,
          new.class_level,
          new.subclass_name,
          v_group,
          ...
          raise exception 'Class feature choice % in group % is not valid for this class, subclass, source, or level.',
            coalesce(v_selection ->> 'name', 'unknown'), v_group_key;
        end if;

        v_flat_choices := v_flat_choices || jsonb_build_array(jsonb_build_object(
          'groupId', v_group_key,
          'groupLabel', coalesce(v_group ->> 'label', ''),
          'groupKind', coalesce(v_group ->> 'kind', ''),
          'level', coalesce((v_group ->> 'level')::integer, 1),
          'key', coalesce(v_selection ->> 'key', ''),
          'name', coalesce(v_selection ->> 'name', ''),
          'source', coalesce(v_selection ->> 'source', ''),
          'kind', coalesce(v_selection ->> 'kind', '')
        ));
      end loop;
    end loop;

    if jsonb_array_length(v_flat_choices) <>
       jsonb_array_length(coalesce(v_sheet -> 'classFeatureChoiceSummary', '[]'::jsonb))
       or exists (
         select 1
         from jsonb_array_elements(v_flat_choices) expected
         where not exists (
           select 1
           from jsonb_array_elements(coalesce(v_sheet -> 'classFeatureChoiceSummary', '[]'::jsonb)) actual
           where actual ->> 'groupId' = expected ->> 'groupId'
             and actual ->> 'key' = expected ->> 'key'
             and private.normalize_player_choice_name_v1(actual ->> 'name') =
                 private.normalize_player_choice_name_v1(expected ->> 'name')
  ...
  if v_invalid_feat_count > 0 or v_missing_feat_count > 0
     or v_sheet_feat_count <> cardinality(v_allowed_feats) then
    raise exception 'Player Forge feats must exactly match their validated background, Species, and class-granted feat sources.';
  end if;
  ...
  if v_sheet_spell_count <> v_actual_spell_count
     or v_invalid_sheet_spell_count > 0
     or v_missing_sheet_spell_count > 0 then
