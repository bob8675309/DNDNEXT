...
for select to authenticated using (
  exists (select 1 from public.encounter_maps m where m.id = map_id and (m.is_active = true or public.is_admin(auth.uid())))
);
...
create or replace function public.admin_upsert_encounter_map_v1(
  p_map_id uuid default null, p_name text default null, p_description text default null,
  p_hex_size integer default 38, p_radius integer default 5,
  p_image_bucket text default null, p_image_path text default null,
  p_is_active boolean default true, p_metadata jsonb default '{}'::jsonb
)
returns uuid language plpgsql security definer
set search_path = pg_catalog, public, private, auth
as $function$
declare v_uid uuid := auth.uid(); v_id uuid := coalesce(p_map_id, gen_random_uuid());
begin
  ...
  insert into public.encounter_maps(id,name,description,image_bucket,image_path,hex_size,radius,metadata,is_active,created_by,updated_at)
  values(v_id,btrim(p_name),nullif(btrim(coalesce(p_description,'')),''),nullif(btrim(coalesce(p_image_bucket,'')),''),nullif(btrim(coalesce(p_image_path,'')),''),p_hex_size,p_radius,coalesce(p_metadata,'{}'::jsonb),coalesce(p_is_active,true),v_uid,timezone('utc',now()))
  on conflict(id) do update set name=excluded.name,description=excluded.description,image_bucket=excluded.image_bucket,image_path=excluded.image_path,hex_size=excluded.hex_size,radius=excluded.radius,metadata=excluded.metadata,is_active=excluded.is_active,updated_at=timezone('utc',now());
  return v_id;
end;$function$;

create or replace function public.admin_set_encounter_hex_v1(
  p_map_id uuid,p_q integer,p_r integer,p_terrain_type text default 'normal',p_movement_multiplier numeric default 1,p_elevation integer default 0,p_hazard_key text default null,p_metadata jsonb default '{}'::jsonb
)
...
begin
  ...
  if p_terrain_type='normal' and coalesce(p_movement_multiplier,1)=1 and coalesce(p_elevation,0)=0 and nullif(btrim(coalesce(p_hazard_key,'')),'') is null and coalesce(p_metadata,'{}'::jsonb)='{}'::jsonb then
    delete from public.encounter_hex_overrides where map_id=p_map_id and q=p_q and r=p_r; return;
  end if;
  ...
  on conflict(map_id,q,r) do update set terrain_type=excluded.terrain_type,movement_multiplier=excluded.movement_multiplier,elevation=excluded.elevation,hazard_key=excluded.hazard_key,metadata=excluded.metadata,updated_at=timezone('utc',now());
end;$function$;

create or replace function public.admin_upsert_encounter_map_object_v1(
  p_object_id uuid default null,p_map_id uuid default null,p_object_type text default null,p_q integer default 0,p_r integer default 0,p_blocks_movement boolean default false,p_blocks_los boolean default false,p_cover_level text default 'none',p_hidden_by_default boolean default false,p_interaction_type text default null,p_state jsonb default '{}'::jsonb,p_metadata jsonb default '{}'::jsonb
)
returns uuid language plpgsql security definer set search_path = pg_catalog, public, private, auth
as $function$
declare v_uid uuid := auth.uid(); v_id uuid := coalesce(p_object_id, gen_random_uuid());
begin
  if coalesce(auth.role(), '') <> 'service_role' and (v_uid is null or not public.is_admin(v_uid)) then raise exception 'Admin required'; end if;
  ...
  on conflict(id) do update set map_id=excluded.map_id,object_type=excluded.object_type,q=excluded.q,r=excluded.r,blocks_movement=excluded.blocks_movement,blocks_los=excluded.blocks_los,cover_level=excluded.cover_level,hidden_by_default=excluded.hidden_by_default,interaction_type=excluded.interaction_type,state=excluded.state,metadata=excluded.metadata,updated_at=timezone('utc',now());
  return v_id;
end;$function$;
