-- Validate player-Forge starting spell selections at transaction end.
-- The existing create_player_character_v2 RPC remains the only creation writer.
...
begin
  ...
  select coalesce(p.cantrips_known, 0), coalesce(p.spells_known, 0), to_jsonb(coalesce(p.spell_slots, array[]::integer[]))
...
drop trigger if exists character_progression_validate_player_forge_spells_v1 on public.character_progression;
create constraint trigger character_progression_validate_player_forge_spells_v1
after insert or update of class_id, class_level
on public.character_progression
deferrable initially deferred
for each row
execute function private.validate_player_forge_starting_spells_v1();
