-- Legacy/recovered Warlocks can have normalized Invocation instances without a sourceChoices object.
-- jsonb_typeof(NULL) <> 'object' is NULL, so the v1 inline guard did not initialize that parent.
-- Route v5 through a wrapper that explicitly uses IS DISTINCT FROM before the v1 mutation.

create or replace function private.ensure_character_source_choices_v1(p_character_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  if jsonb_typeof(v_sheet->'sourceChoices') is distinct from 'object' then
...
create or replace function private.apply_level_up_warlock_invocation_replacement_v2(
  p_character_id uuid,
  p_to_level integer,
  p_group jsonb,
...
begin
  ...
  return private.apply_level_up_warlock_invocation_replacement_v1(
    p_character_id,p_to_level,p_group,p_all_class_selections,p_feat_instances
  );
end;
$$;
...
revoke all on function private.ensure_character_source_choices_v1(uuid) from public,anon,authenticated;
...
revoke all on function private.apply_level_up_warlock_invocation_replacement_v2(uuid,integer,jsonb,jsonb,jsonb) from public,anon,authenticated;
revoke all on function public.complete_character_level_up_v5(uuid,jsonb) from public,anon;
grant execute on function private.ensure_character_source_choices_v1(uuid) to service_role;
grant execute on function private.apply_level_up_warlock_invocation_replacement_v2(uuid,integer,jsonb,jsonb,jsonb) to service_role;
