-- Guarded starting-magic wrapper for the shared Player Character Forge.
-- The existing v2 RPC remains the compatibility/base-class creation primitive.
-- v3 supplies legal proxy class-list rows only when a Background-expanded spell
-- must satisfy v2's class spell-count checks, then replaces that transient set
-- with the exact validated multi-source starting selections before deferred
-- authority triggers run at transaction end.
...
begin
  ...
  -- Preserve all real base-class choices already accepted by v2.
  ...
  -- For each Background-expanded spell that consumes a normal class spell
  -- selection, provide v2 a same-level canonical class-list proxy. This row is
  -- transaction-local scaffolding only; all rows are replaced below.
  for v_choice in
  ...
  -- Replace v2's temporary/base rows with the exact selections validated below.
  ...
  loop
    ...
    if not found then
      raise exception 'Starting magic selection references an unavailable spell.';
    end if;
    ...
    if v_source_type = 'class' then
      ...
      if not (
        (lower(v_class_name) = 'fighter' and lower(v_subclass_name) = 'eldritch knight')
        or (lower(v_class_name) = 'rogue' and lower(v_subclass_name) = 'arcane trickster')
      ) then
        raise exception 'Subclass spell source is not valid for % / %.', v_class_name, v_subclass_name;
      end if;
      if not ('Wizard' = any(coalesce(v_spell.classes, '{}'::text[]))) then
    ...
    insert into public.character_spells(character_id, spell_id, source_type, source_key, known, prepared)
    values (v_character_id, v_spell.id, v_source_type, v_source_key, true, v_prepared)
...
grant execute on function public.create_player_character_v3(jsonb, jsonb, jsonb) to authenticated;

-- The deferred sheet/spell exactness check now covers every authoritative
-- starting spell row materialized by the guarded Forge, not only base-class rows.
...
begin
  ...
  select count(*) into v_actual_count from public.character_spells where character_id = new.character_id;

  if v_expected_count <> v_actual_count then
    raise exception 'Starting magic summary does not match authoritative character spell rows.';
  end if;

  if exists (
    select 1
    from jsonb_array_elements(v_expected) expected
    where not exists (
      select 1
    ...
    raise exception 'Starting magic summary contains a spell/source/prepared state that was not materialized.';
  end if;
...
drop trigger if exists character_progression_validate_player_forge_starting_magic_exactness_v1 on public.character_progression;
create constraint trigger character_progression_validate_player_forge_starting_magic_exactness_v1
after insert or update on public.character_progression
deferrable initially deferred
for each row execute function private.validate_player_forge_starting_magic_exactness_v1();
