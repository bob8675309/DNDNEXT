-- Keep the existing Review Level Up button compatible while the UI migrates
-- to progression v3. Epic Boon is now source-modeled, so it is no longer an
-- unsupported level-19 blocker when it is the only flagged class choice.
...
begin
  ...
  if v_requires_epic and jsonb_array_length(v_unsupported)=1 and lower(v_unsupported->>0)='epic boon' then
    v_unsupported:='[]'::jsonb;
  end if;
  ...
  if v_requires_subclass then
    v_required:=v_required||jsonb_build_array(jsonb_build_object('key','subclass_name','type','text','label','Subclass','required',true));
  end if;
  if v_requires_advancement or v_requires_epic then
    v_required:=v_required||jsonb_build_array(jsonb_build_object(
      'key','advancement',
      'type',case when v_requires_epic then 'epic_boon_or_feat' else 'asi_or_feat' end,
      'label',case when v_requires_epic then 'Epic Boon or General Feat' else 'Ability Score Improvement or Feat' end,
      'required',true
    ));
  end if;
  ...
  v_preview:=jsonb_build_object(
    ...
    'fromLevel',v_progression.class_level,'toLevel',v_progression.class_level+1,
    'xp',v_progression.experience_points,'requiredXp',v_next.xp_threshold,'proficiencyBonus',v_next.proficiency_bonus,
    'cantripsKnown',v_next.cantrips_known,'spellsKnown',v_next.spells_known,'spellSlots',v_next.spell_slots,
    'highestSpellLevel',v_highest_spell_level,'newCantrips',v_new_cantrips,'newLeveledSpells',v_new_leveled,
    'features',v_next.features,'choices',v_required,'unsupportedChoices',v_unsupported,
  ...
  if v_session.id is null then
    ...
    set from_level=v_progression.class_level,to_level=v_progression.class_level+1,metadata_ready=v_metadata_ready,
        required_choices=v_required,preview=v_preview,updated_at=now()
  ...
  if v_created then
    ...
    values(p_character_id,'level_up_review_started',v_progression.class_level,v_progression.class_level+1,
  ...
  return jsonb_build_object(
    'session',to_jsonb(v_session),'preview',v_preview,'metadataReady',v_metadata_ready,'canComplete',v_metadata_ready,
