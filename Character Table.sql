create table public.characters (
  ...
  sprite_key text null,
  map_scale numeric null,
  sprite_path text null,
  sprite_scale double precision null,
  constraint characters_pkey primary key (id),
  constraint characters_name_key unique (name),
  constraint characters_location_id_fkey foreign KEY (location_id) references locations (id) on delete set null,
  constraint characters_map_icon_id_fkey foreign KEY (map_icon_id) references map_icons (id),
  constraint characters_home_location_id_fkey foreign KEY (home_location_id) references locations (id) on delete set null,
  constraint characters_last_known_location_id_fkey foreign KEY (last_known_location_id) references locations (id) on delete set null,
  constraint characters_projected_destination_id_fkey foreign KEY (projected_destination_id) references locations (id) on delete set null,
...
create index IF not exists idx_characters_home_location_id on public.characters using btree (home_location_id) TABLESPACE pg_default;
