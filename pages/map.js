...
import LocationSideBar from "../components/LocationSideBar"; // (not mounted here, but kept for parity)
 ...
 *
 * NOTE ABOUT DIM OVERLAY & OFFCANVAS Z-INDEX
 * We toggle a lightweight dim (map-dim) whenever either offcanvas is shown.
 * That depends on the Offcanvas events actually firing. Some builds of the app
 * were loading Bootstrap CSS but not the JS, which prevented .show() from
 * working. This file:
 *   1) Uses window.bootstrap if present (ideal),
 *   2) Falls back to simply toggling the .show class on the panel node so the
 *      user can still interact.
 *
 * IMPORTANT: temporary X calibration so legacy coordinates keep matching spots
 * when the map width used to be stretched. Set SCALE_X = 1 once all pins are
 * re-saved.
 ********************************************************************* */
...
export default function MapPage() {
  ...
  const [panelOpen, setPanelOpen] = useState(false); // controls gray dim
  ...
  async function loadMerchants() {
    const { data, error } = await supabase
      .from("merchants")
      .select(
        "id,name,x,y,inventory,icon,location_id,last_known_location_id,projected_destination_id,created_at"
      )
  ...
  function showOffcanvasById(id) {
    ...
    // Preferred: Bootstrap JS present
    if (window.bootstrap?.Offcanvas) {
      const oc = window.bootstrap.Offcanvas.getOrCreateInstance(node, {
        backdrop: false,
        scroll: true,
        keyboard: true,
      });
    ...
    // Fallback: toggle classes so the panel still appears
    node.classList.add("show");
    node.style.visibility = "visible";
    setPanelOpen(true);
    const onKey = (e) => {
      if (e.key === "Escape") hideOffcanvasFallback();
    };
    ...
    // close button already has data-bs-dismiss; emulate
    const closeBtn = node.querySelector(".btn-close");
    if (closeBtn) closeBtn.addEventListener("click", hideOffcanvasFallback, { once: true });
  ...
  function openLocationPanel(l) {
    setSel(l);
    hydrateLocation(l);
    showOffcanvasById("locPanel");
  }

  function openMerchantPanel(m) {
    setSelMerchant(m);
    showOffcanvasById("merchantPanel");
  }
  ...
  async function createLocation(fd) {
    const patch = {
      ...
      x:
        clickPt?.x != null
          ? String(Math.round((clickPt.x / SCALE_X) * 10) / 10)
          : null, // store de-calibrated x
  ...
  function handleMapClick(e) {
    ...
    if (addMode) {
      ...
      if (window.bootstrap?.Modal && el) {
        ...
        // fallback: reveal the modal node
              ...
              <div key={q.id} className="loc-item">
                ...
                {isAdmin && (
                  <div className="mt-2 d-flex gap-2 small">
                    <button className="btn btn-link p-0" data-bs-toggle="modal" data-bs-target={`#editQuestModal-${q.id}`}>
                      Edit
                    </button>
                    <button
          ...
          <div className="loc-sec">
            ...
            {selNPCs.map((n) => (
              <div key={n.id} className="loc-item">
                ...
                {isAdmin && (
                  <div className="mt-2 d-flex gap-2 small">
                    <button className="btn btn-link p-0" data-bs-toggle="modal" data-bs-target={`#editNpcModal-${n.id}`}>
                      Edit
                    </button>
                    <button
                      className="btn btn-link text-danger p-0"
                    ...
                    <div className="modal-dialog">
                      ...
                      >
                        ...
                        <div className="modal-body">
                          <div className="mb-2">
                            <label className="form-label">Name</label>
                            <input name="name" className="form-control" defaultValue={n.name} required />
                          </div>
                          <div className="mb-2">
                            <label className="form-label">Race</label>
                            <input name="race" className="form-control" defaultValue={n.race || ""} />
                          </div>
                          <div className="mb-2">
                            <label className="form-label">Role</label>
                            <input name="role" className="form-control" defaultValue={n.role || ""} />
                          </div>
                        </div>
                        <div className="modal-footer">
                          <button className="btn btn-secondary" data-bs-dismiss="modal">
        ...
        <>
          ...
          <div className="modal fade" id="createNpcModal" tabIndex={-1} aria-hidden="true">
            <div className="modal-dialog">
              ...
              >
                <div className="modal-header">
                  <h5 className="modal-title">Create NPC</h5>
                  <button className="btn-close" data-bs-dismiss="modal" />
                </div>
                <div className="modal-body">
                  <div className="mb-2">
          ...
          <div className="modal fade" id="createQuestModal" tabIndex={-1} aria-hidden="true">
            <div className="modal-dialog">
              ...
              >
                <div className="modal-header">
                  <h5 className="modal-title">Create Quest</h5>
                  <button className="btn-close" data-bs-dismiss="modal" />
                </div>
                <div className="modal-body">
                  <div className="mb-2">
