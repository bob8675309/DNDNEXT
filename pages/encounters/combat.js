...
export default function EncounterCombatPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    if (!active?.id) { setCanControl(false); return undefined; }
    supabase.rpc("encounter_can_control_participant_v1", { p_participant_id: active.id }).then(({ data, error }) => { if (!cancelled) setCanControl(!error && Boolean(data)); });
  ...
  return (
    <main className="combat-page">
      <header className="combat-header">
        <div><div className="kicker">TACTICAL ENCOUNTER • PHASE 1E</div><h1>Core Combat</h1><p>Actions and attack outcomes are resolved by the server. Movement remains available on the Turn Movement surface.</p></div>
      ...
      <section className="layout">
        <aside className="sidebar">
          ...
          {active ? <div className="panel"><div className="kicker">First attack path</div><h2>Unarmed Strike</h2><select value={targetId} onChange={(e) => setTargetId(e.target.value)}><option value="">Choose target</option>{targets.map((p) => <option key={p.id} value={p.id}>{p.display_name} • {p.team} • HP {p.current_hp ?? "?"}</option>)}</select>{target ? <div className="read"><span>Range</span><strong>{targetDistance * 5} ft.</strong></div> : null}<button className="attack" onClick={unarmedStrike} disabled={!canControl || !active.action_available || !target || targetDistance > 1 || saving}>Unarmed Strike</button><p>Strength + proficiency vs target AC. A Dodging target imposes disadvantage. Damage is encounter-local.</p></div> : null}
        </aside>
