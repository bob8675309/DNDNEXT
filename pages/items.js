...
function RecipeTable({ recipes, selected, onSelect }) {
  return <div className="craft-table-wrap"><table className="craft-recipe-table"><thead><tr><th>Recipe</th><th>Discipline</th><th>Kind</th><th>Rarity</th><th>Applies / Family</th><th>Slot</th><th>Status</th><th>Source</th></tr></thead><tbody>{recipes.map((recipe) => <tr key={recipe.id} className={selected?.id === recipe.id ? "active" : ""} onClick={() => onSelect(recipe)}><td><div className="craft-table-name">{recipe.name}</div><div className="craft-table-sub">{recipe.summary || "—"}</div></td><td>{recipe.discipline || "—"}</td><td>{titleCase(recipe.kind || "recipe")}</td><td><span className="craft-badge">{recipe.rarity || "—"}</span></td><td>{recipe.family || recipe.category || "—"}</td><td>{recipeSlotLabel(recipe)}</td><td><span className={cls("craft-badge", recipe.known && "craft-badge-known")}>{recipe.known ? "Known" : "Reference"}</span></td><td>{recipe.source || "—"}</td></tr>)}{!recipes.length ? <tr><td colSpan="8" className="text-muted p-3">No recipes found.</td></tr> : null}</tbody></table></div>;
}
